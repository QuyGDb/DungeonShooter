...
public class MusicManager : SingletonMonobehaviour<MusicManager>
{
    ...
    {
        // if fade out routine already runninng then stop it
        if (fadeOutMusicCoroutine != null)
        {
            StopCoroutine(fadeOutMusicCoroutine);
        }

        // if fade in routine already runninng then stop it
    ...
    private void SetMusicVolume(int volume)
    {
        ...
        {
            GameResources.Instance.musicMasterMixerGroup.audioMixer.SetFloat("musicVolume", HelperUtilities.LinearToDecibels(volume));
        }
    }
}
